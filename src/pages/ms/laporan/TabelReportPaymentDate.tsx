    ...
    img.onload = () => {
      ...
      const tableBody = [
        [
          ...
          formattedUpdatedAt,
          `Rp ${parseFloat(data.total_payment).toLocaleString('id-ID', { maximumFractionDigits: 0 })}`
        ]
      ]
      doc.autoTable({
        startY: studentInfoY + infoLines.length * lineSpacing + 10,
        head: [['ID', 'Pembayaran', 'Tipe', 'Bulan', 'Status', 'Jenis Bayar', 'Tanggal', 'Total']],
        body: tableBody,
        theme: 'grid',
...
const columns: GridColDef[] = [
  ...
  { field: 'unit_name', headerName: 'Unit', flex: 0.175, minWidth: 80 },
  { field: 'full_name', headerName: 'Nama Siswa', flex: 0.175, minWidth: 300 },
  ...
  { field: 'metode_pembayaran', headerName: 'Jenis Pembayaran', flex: 0.175, minWidth: 180 },
...
const TabelReportPaymentDate = ({ school_id, date_first, date_last, status }: TabelReportPaymentDateProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        await dispatch(ListPaymentReportDate({ school_id, date_first, date_last, q: value, status }))
      } catch (error) {
    ...
    fetchData()
  }, [dispatch, school_id, date_first, date_last, status, value])
  ...
  const createPdf = async () => {
    ...
    if (store.data && store.data.length > 0) {
      const pdfData: any = store.data[0]
      const logoImageUrl = '/images/logo.png'
      const img = new Image()
      img.src = logoImageUrl
      ...
      img.onload = () => {
        ...
        // Nama sekolah dan alamat (centered)
        doc.setFontSize(14)
        doc.setFont('verdana', 'normal')
        ...
        infoLines.forEach((info, index) => {
          const y = studentInfoY + Math.floor(index / 2) * lineSpacing
          if (index % 2 === 0) {
            doc.text(info.label, leftColumnX, y)
            doc.text(`: ${info.value}`, leftColumnX + labelOffset, y)
          } else {
            doc.text(info.label, rightColumnX, y)
            doc.text(`: ${info.value}`, rightColumnX + labelOffset, y)
          }
        })

        // Garis bawah info
        const endInfoY = studentInfoY + lineSpacing + 2
        doc.line(10, endInfoY, 200, endInfoY)

        // Judul tabel
        doc.text('Dengan rincian pembayaran sebagai berikut:', 10, endInfoY + 10)
        ...
        store.data.forEach((item: any) => {
          ...
          tableBody.push([
            ...
            formattedUpdatedAt,
            `Rp ${parseFloat(item.total_payment).toLocaleString('id-ID', { maximumFractionDigits: 0 })}`
          ])
